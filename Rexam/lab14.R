...
mpg <- as.data.frame(ggplot2::mpg) #ggplot2::mpg은 tibble 타입이다. 
...
table(click_count)
ggplot(data=click_count, aes(x=V2)) + labs(title = "제품당 클릭수", subtitle="제품당 클릭수를 바그래프로 표현", x="상품ID", y="클릭수") + geom_bar(aes(fill=V2)) #aes(fill=V2)을 geom_bar에다가 하면 여기에만 적용되는데 ggplot에다가 하면 그 그래프에 오는 모든 geom_시리즈에 적용된다.
