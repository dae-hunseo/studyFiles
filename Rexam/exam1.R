...
# logical값(TRUE/FALSE)을 활용한 인덱싱
x[c(2,4)] # x[2], x[4]
x[c(F,T,F,T,F)] # x[c(T,F)] #왼쪽 주석처럼 하면 T, F, T, F, T로 된다.
x > 5
x[x > 5] #벡터에서 원소값이 5보다 큰 원소값을 추출
...
x
names(x)
names(x) <- LETTERS[1:5] #LETTERS는 대문자 A~Z를 가지고 있는 객체. 그 중에 5번째까지인 A, B, C, D, E다.
x
x[2];x["B"]  #숫자 인덱스로 꺼내고, 원소 이름으로 꺼냄
names(x) <- NULL
x
x[2];x["B"] 
...
c(T, T, F, F) & c(T, F, T, F)
c(T, T, F, F) | c(T, F, T, F)
c(T, T, F, F) && c(T, F, T, F) #첫번째 엘리먼트만 비교하고 TRUE나 FALSE를 출력한다.
c(T, T, F, F) || c(T, F, T, F) #첫번째 엘리먼트만 비교하고 TRUE나 FALSE를 출력한다.
...
# which(), which.max(), which.min() 함수 익히기 #which()는 인덱스를 출력하는 함수다. which.max()는 가장 큰 값을 갖고있는 데이터의 인덱스를, which.min()은 가장 작은 값을 갖고있는 데이터의 인덱스를 출력한다.
...
paste("I'm","Duli","!!", sep="")
paste("I'm","Duli","!!", sep="@")
paste0("I'm","Duli","!!") #paste0는 paste함수에서 setp=""를 해준 것 처럼 각각의 원소를 공백없이 이어준다.


# paste() 함수에 어려 백터들을 아규먼트로 주면 백터들의 원소끼리 결합하여 백터 구성 
fruit <- c("Apple", "Banana", "Strawberry")
food <- c("Pie","Juice", "Cake")
paste(fruit, food) #fruit객체와 food객체를 각각 같은 인덱스끼리 결합해준다.
